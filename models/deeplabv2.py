...
class ResNet101(nn.Module):
    def __init__(self, block, layers, num_classes, BatchNorm, classfier_module, bn_clr=False):
        self.inplanes = 64
        self.bn_clr = bn_clr
        ...
        self.layer5 = self._make_pred_layer(classfier_module, 2048, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
...
def Deeplab(BatchNorm=nn.BatchNorm2d, num_classes=19, freeze_bn=False, restore_from=None, initialization=None, bn_clr=False, is_teacher=False):
    if is_teacher:
        model = ResNet101(Bottleneck, [3, 4, 23, 3], num_classes, BatchNorm, bn_clr=bn_clr, classfier_module=Classifier_Module)
    else:
        model = ResNet101(Bottleneck, [3, 4, 23, 3], num_classes, BatchNorm, bn_clr=bn_clr, classfier_module=Classifier_Module2)
    return model